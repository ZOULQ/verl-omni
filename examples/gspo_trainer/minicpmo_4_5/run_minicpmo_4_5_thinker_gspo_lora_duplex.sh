#!/usr/bin/env bash
# MiniCPM-o 4.5 Thinker GSPO + LoRA training — Path 2 (native duplex data plane).
#
# Same whole-network deployment and Thinker-only training as the Path 1 script,
# but the rollout drives the in-process vLLM-Omni engine's native duplex control
# plane over the server's /v1/realtime WebSocket endpoint through the
# `minicpmo_duplex_agent` agent loop instead of the batched `generate` path.
#
# Requires the two vLLM-Omni logprob changes (§6.1 Thinker native-duplex sampler
# logprob + §6.2 data-plane forwarding) and the verl-omni duplex client/agent
# loop (verl_omni/workers/rollout/vllm_rollout/vllm_omni_duplex_client.py and
# verl_omni/pipelines/minicpmo_4_5/duplex_agent_loop.py).
#
# The dataset must provide the question audio as the FIRST audio entry and the
# reference voice as the SECOND audio entry in multi_modal_data["audios"].
...
python3 -m verl_omni.trainer.main_omni \
    ...
    actor_rollout_ref.rollout.agent.default_agent_loop=minicpmo_duplex_agent \
    ...
    trainer.experiment_name=minicpmo_4_5_thinker_duplex_path2 \
